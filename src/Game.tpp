...
void Game::iniciar_juego()
{
    ...
    std::cout << totalFichas << std::endl << std::endl;
    int numero = 0;
    //cambiar_turno(actual); //totalFichas
    Jugador actual = jugadores_en_juego_queue.dequeue();

    //pintar por primera vez el tablero.
    tablero.pintar_tablero();
    while (palabras_len != palabras_jugadas.size() && fichas_en_tablero < totalFichas)
    {
...
int Game::busqueda_horizontal(Tablero& _tablero, const std::string& palabra)
{
    ...
    {
        size_t columna = 0, contador = 0;
        while (columna < Tablero::COLUMNAS)
        {
            if (contador == 0 && columna > Tablero::COLUMNAS - palabra.length())
            {
                break;
            }
            ...
            char simbolo = _tablero.casillas[fila][columna]->obtenerSimbolo();
            if (simbolo == ' ' || simbolo == '#' || simbolo != palabra[contador])
            {
                temp = 1;
                puntos = 0;
                contador = 0;
